...
def _clamp(value: float, low: float = 0.0, high: float = 1.0) -> float:
    """Constrain a score to the closed interval [low, high]."""
...
class RAGASEvaluator:
    ...
    def evaluate_faithfulness(self, answer: str, context: str) -> float:
        ...
        if not answer_tokens:
            # Nothing was claimed, so nothing can be ungrounded.
            return 1.0
        context_tokens = _tokenize(context)
...
class LLMJudge:
    ...
    #: Judge scores below this average look suspiciously harsh.
    SEVERITY_THRESHOLD: float = 0.3
    #: Judge scores above this average look suspiciously generous.
    LENIENCY_THRESHOLD: float = 0.8
    #: How much the first item must lead the rest before we call it positional.
    ...
    def score_response(
        ...
        # Every rubric criterion must be reported, even when the judge skipped it.
        for name in rubric:
    ...
    def _build_prompt(question: str, answer: str, rubric: dict[str, Any]) -> str:
        """Build the judge prompt: question + answer + explicit rubric."""
        criteria = "\n".join(f"- {name}: {desc}" for name, desc in rubric.items())
        criterion_names = ", ".join(f'"{name}"' for name in rubric) or '"overall"'
        return f"""You are an impartial evaluator of an AI assistant's answer.
Score the answer against every rubric criterion below.

Rubric (score each criterion from 1 to 5):
{criteria or "- overall: Is the answer correct, complete and relevant?"}

Scoring anchors:
  5 = Correct, complete, grounded in the cited policy.
  4 = Mostly correct, minor gaps.
  3 = Partially correct, some errors.
  2 = Significant errors or missing conditions.
  1 = Wrong, irrelevant, or unsupported by evidence.

Judge only the content. Length is not quality: do not reward a longer answer
that adds no supported information, and do not penalise a short answer that is
complete.
...
Respond with JSON only, using the keys {criterion_names} plus "reasoning":
{{"criterion": <score>, ..., "reasoning": "<one sentence per criterion>"}}"""

    @staticmethod
    def _parse_scores(raw_text: str) -> dict[str, float]:
        """Extract criterion → score (normalised to 0-1) from a judge reply."""
        payload = LLMJudge._extract_json_object(raw_text)
        if payload is None:
            return {}
    ...
    def _extract_json_object(raw_text: str) -> dict[str, Any] | None:
        """Return the first JSON object in the text, or None when unparseable."""
        try:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        """
        per_item = [self._mean_score(item) for item in scores_batch or []]
        per_item = [value for value in per_item if value is not None]
        if not per_item:
            return {
        ...
        overall_average = sum(per_item) / len(per_item)
        positional_bias = False
        if len(per_item) >= 2:
            rest = per_item[1:]
            positional_bias = (
                per_item[0] - max(rest) > self.POSITIONAL_MARGIN
                and per_item[0] - (sum(rest) / len(rest)) > self.POSITIONAL_MARGIN
            )

        return {
            "positional_bias": positional_bias,
            "leniency_bias": overall_average > self.LENIENCY_THRESHOLD,
            "severity_bias": overall_average < self.SEVERITY_THRESHOLD,
        }

    @staticmethod
    def _mean_score(item: Any) -> float | None:
        """Average the criterion scores of one score_response() payload."""
...
class BenchmarkRunner:
    ...
    #: A metric average may not fall more than this below the baseline.
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            # Keep the caller's pair (it carries id/difficulty metadata).
            result.qa_pair = pair
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_faithfulness": self._average(
                [result.faithfulness for result in results]
            ),
            "avg_relevance": self._average([result.relevance for result in results]),
            "avg_completeness": self._average(
                [result.completeness for result in results]
            ),
            "avg_context_recall": self._optional_average(
                [result.context_recall for result in results]
            ),
            "avg_context_precision": self._optional_average(
    ...
    def _optional_average(values: list[float | None]) -> float | None:
        """Average the scored results only; None when nothing was scored."""
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        for metric in metrics:
            new_avg = self._average(
                [getattr(result, metric) for result in new_results]
            )
            baseline_avg = self._average(
...
class FailureAnalyzer:
    ...
    #: One concrete, prioritised action per failure category.
    SUGGESTION_BY_TYPE: dict[str, str] = {
        "hallucination": (
            "Add a grounding check that rejects answer sentences whose claims are "
            "absent from the retrieved contexts, and require the generator to cite "
            "the source document for every policy statement"
        ),
        "irrelevant": (
            "Sharpen the answer prompt so the model restates and addresses every "
            "part of the question, and add few-shot examples of multi-part "
            "student-services answers"
        ),
        "incomplete": (
            "Raise retrieval top_k and chunk size so dates, amounts, conditions and "
            "exceptions stay in one context, and instruct the generator to keep "
            "every numeric condition it finds"
        ),
        "off_topic": (
            "Add intent routing before generation so scope, policy-version and "
            "procedural questions use the right document set"
        ),
        "refusal": (
            "Relax the guardrail so it only refuses genuinely out-of-scope or "
            "privacy-sensitive requests instead of in-corpus policy questions"
        ),
    }

    #: Fallback actions so the log always carries a prioritised backlog.
    GENERIC_SUGGESTIONS: list[str] = [
        "Rerank retrieved chunks by query overlap to push the relevant evidence "
        "above noise and raise Context Precision",
        "Add the failing questions to the regression benchmark so the next run "
        "detects a repeat of the same root cause",
        "Calibrate the evaluation thresholds against human labels before using "
        "them as a CI/CD quality gate",
    ]
    ...
    def categorize_failures(
        ...
        counts: Counter[str] = Counter(
            failure.failure_type or "unknown" for failure in failures or []
        )
        return dict(counts)
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures or []):
            ...
            # Index-matched suggestion when one exists; otherwise fall back to
            # the action for this failure category (never repeat an unrelated
            # suggestion just to fill the column).
            if index < len(suggestions):
                fix = suggestions[index]
            else:
                fix = self.SUGGESTION_BY_TYPE.get(
                    failure_type,
                    f"Investigate the {failure_type} cluster and add a targeted "
                    "guardrail",
                )
            lines.append(
    ...
    def generate_improvement_suggestions(
        ...
        # Most frequent failure category first — one root-cause fix clears a cluster.
        for failure_type, count in sorted(
            counts.items(), key=lambda item: (-item[1], item[0])
        ):
            action = self.SUGGESTION_BY_TYPE.get(failure_type)
            if action is None:
                action = (
                    f"Investigate the {failure_type} cluster and add a targeted "
                    "guardrail"
                )
            suggestions.append(f"[{count} case(s)] {action}")

        for fallback in self.GENERIC_SUGGESTIONS:
